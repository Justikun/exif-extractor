...
func parseIFD(br *metadata.BinaryReader, tiffHeaderStart int64, ifdType metadata.IFDtype, endian binary.ByteOrder) error {
	...
	for range tagCount {
		...
		if totalTagDataSize > 4 {
			...
			if err != nil { return fmt.Errorf("Failed to save current position\n") }

			// jump to offset
			_, err = br.Seek(absDataOffset, io.SeekStart)
			if err != nil {	return fmt.Errorf("Failed to seek to app1 data tag offset\n") }

			// read data in bytes
			dataInBytes, err := br.ReadBytes(int(totalTagDataSize))
			if err != nil {	return fmt.Errorf("Failed to read dataInBytes\n") }
